...
def create_mongo_prompt(platform: str, question: str) -> str:
    ...
    return f"""
You are a world-class MongoDB data analyst expert.
Your sole task is to convert a natural language question into a secure, read-only MongoDB aggregation pipeline.
**CONSTRAINTS (VERY IMPORTANT):**
1.  **READ-ONLY:** You MUST ONLY generate pipelines for reading data.
Absolutely NO commands for `update`, `delete`, `drop`, `insert`, `createUser`, or any other data modification or user management.
Any question asking to modify data must be rejected by returning an empty array `[]`.
2.  **AGGREGATION ONLY:** The output must be a valid MongoDB aggregation pipeline (an array of stages).
3.  **JSON ONLY:** Your entire response must be ONLY the raw JSON array.
...
def platform_nl_query(
    ...
    """
    [cite_start]Natural language query handler for Google, Meta, Shopify, etc. [cite: 87]
    Example: POST /analytics/google_ads
    Body: { "question": "Which 5 campaigns had the highest spend last week?" [cite_start]} [cite: 87]
    """
    question = request.get("question")
    if not question:
        raise HTTPException(status_code=400, detail="Missing question")
    ...
    # 1️⃣ Generate MongoDB Pipeline with Gemini
    model = genai.GenerativeModel('gemini-pro') # Using Flash for speed and cost-efficiency
    llm_prompt = create_mongo_prompt(platform, question)
